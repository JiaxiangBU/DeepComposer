...
class lstm_double:
    ...
    def get_dynamicRNN(self, batch_size):
        ...
        self.batch_x = tf.placeholder(tf.float32, [None, batch_size, self.input_dim])
        self.batch_y = tf.placeholder(tf.float32, [None, batch_size, self.input_dim])
        ...
        self.second_out, _ = tf.nn.dynamic_rnn(self.second_cell,
                                               first_out,
                                               sequence_length=self.seq_length,
    ...
    def setTrainer(self):
        self.loss = tf.reduce_mean(tf.square(self.second_out - self.batch_y))
